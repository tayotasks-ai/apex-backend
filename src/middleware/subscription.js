const { AcademicSession, Subscription } = require('../models');
const { ApiError } = require('../utils/helpers');

/**
 * Gates all protected school routes.
 ...
 * - Blocks with 402 Payment Required if a session exists but no active subscription
 */
  ...
  try {
    ...
    const err = new ApiError(402,
      sub?.status === 'pending'
        ...
        : `Subscription required for "${session.name}". Please pay ₦2,000 per student to continue.`
    );
